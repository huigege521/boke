...
class Cookie implements ArrayAccess, CloneableCookieInterface
{
    ...
    public static function setDefaults($config = [])
    {
        ...
        if ($config instanceof CookieConfig) {
            $newDefaults = [
                'prefix'   => property_exists($config, 'prefix') ? $config->prefix : self::$defaults['prefix'],
                'expires'  => property_exists($config, 'expires') ? $config->expires : self::$defaults['expires'],
                'path'     => property_exists($config, 'path') ? $config->path : self::$defaults['path'],
                'domain'   => property_exists($config, 'domain') ? $config->domain : self::$defaults['domain'],
                'secure'   => property_exists($config, 'secure') ? $config->secure : self::$defaults['secure'],
                'httponly' => property_exists($config, 'httponly') ? $config->httponly : self::$defaults['httponly'],
                'samesite' => property_exists($config, 'samesite') ? $config->samesite : self::$defaults['samesite'],
                'raw'      => property_exists($config, 'raw') ? $config->raw : self::$defaults['raw'],
            ];
