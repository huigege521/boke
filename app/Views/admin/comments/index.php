            ...
            <ul class="nav nav-tabs">
                <li class="nav-item">
                    <a class="nav-link <?= !isset($status) ? 'active' : '' ?>"
                        href="<?= base_url('admin/comments') ?>">全部</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link <?= isset($status) && $status == 'pending' ? 'active' : '' ?>"
                        href="<?= base_url('admin/comments/pending') ?>">待审核</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link <?= isset($status) && $status == 'approved' ? 'active' : '' ?>"
                        href="<?= base_url('admin/comments/approved') ?>">已通过</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link <?= isset($status) && $status == 'spam' ? 'active' : '' ?>"
                        href="<?= base_url('admin/comments/spam') ?>">垃圾</a>
                </li>
            </ul>
        </div>
        <div class="col-md-8">
            <form action="<?= isset($status) ? '/admin/comments/' . $status : '/admin/comments' ?>" method="get"
                class="form-inline float-right">
                <div class="input-group">
                    <input type="text" name="search" class="form-control" placeholder="搜索评论..."
...
<!-- 批量操作 -->
<form action="<?= base_url('admin/comments/batchAction') ?>" method="post" class="mb-4">
    <div class="row">
        <div class="col-md-4">
                    ...
                    <tbody>
                        <?php foreach ($comments as $comment): ?>
                            <tr>
                                ...
                                <td><a href="<?= base_url('post/' . ($comment['slug'] ?? '')) ?>"
                                ...
                                <td>
                                    <a href="<?= base_url('admin/comments/' . $comment['id'] . '/edit') ?>"
                                        class="btn btn-sm btn-primary">编辑</a>
                                    <form action="<?= base_url('admin/comments/' . $comment['id']) ?>" method="post"
