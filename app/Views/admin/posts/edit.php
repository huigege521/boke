    ...
    <div class="card-body">
        <form action="/admin/posts/<?= $post['id'] ?>" method="post" enctype="multipart/form-data">
                ...
                <?php if ($post['featured_image']): ?>
                    <div class="current-image mt-2">
                        ...
                        if (in_array($fileExtension, ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
                            // 显示图片
                            echo '<img src="' . $filePath . '" alt="Featured Image" style="max-width: 200px; height: auto;">';
                        } else if ($fileExtension == 'pdf') {
                            // 显示 PDF 链接
                            echo '<a href="' . $filePath . '" target="_blank" class="btn btn-sm btn-primary">查看 PDF 文件</a>';
                        } else {
                            // 显示其他文件链接
                            echo '<a href="' . $filePath . '" target="_blank" class="btn btn-sm btn-secondary">下载文件</a>';
                        }
                        ?>
            ...
            <div class="form-group">
                ...
                <textarea id="excerpt" name="excerpt" rows="5"><?= old('excerpt', $post['excerpt']) ?></textarea>
            ...
            <div class="form-group">
                ...
                <select id="status" name="status">
                    ...
                    <option value="published" <?= old('status', $post['status']) == 'published' ? 'selected' : '' ?>>已发布
                    </option>
                    <option value="pending" <?= old('status', $post['status']) == 'pending' ? 'selected' : '' ?>>待审核
                    </option>
                    <option value="scheduled" <?= old('status', $post['status']) == 'scheduled' ? 'selected' : '' ?>>定时发布
                    </option>
                </select>
            </div>

            <div class="form-group" id="scheduled_at_container"
                style="display: <?= old('status', $post['status']) == 'scheduled' ? 'block' : 'none' ?>">
                <label for="scheduled_at">定时发布时间</label>
                <input type="datetime-local" id="scheduled_at" name="scheduled_at"
                    value="<?= old('scheduled_at', $post['scheduled_at'] ? date('Y-m-d\TH:i', strtotime($post['scheduled_at'])) : '') ?>">
            </div>

            <div class="form-group">
                <label for="visibility">可见性</label>
                <select id="visibility" name="visibility">
                    <option value="public" <?= old('visibility', $post['visibility']) == 'public' ? 'selected' : '' ?>>公开
                    </option>
                    <option value="private" <?= old('visibility', $post['visibility']) == 'private' ? 'selected' : '' ?>>私有
                    </option>
                </select>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        ClassicEditor
            .create(document.querySelector("#content"), {
                ...
                // 只禁用 MediaEmbed 插件，避免 Selection 错误
                ...
                editor.model.document.on('change:data', function () {
                    ...
                    autoSaveTimer = setTimeout(() => {
                        ...
                        if (content !== lastSavedContent) {
                            ...
                            fetch('/admin/posts/auto-save', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/x-www-form-urlencoded'
                                },
                                body: 'post_id=<?= $post['id'] ?>&content=' + encodeURIComponent(content) + '&csrf_token=' + csrfToken
                            })
                                .then(response => {
                                    // 检查响应头是否包含新的CSRF令牌
                                ...
                                .then(data => {
                                    if (data.success) {
                                        console.log('自动保存成功:', data.auto_saved_at);
                                        showAutoSaveMessage('草稿已自动保存');
                                    } else {
                                        showAutoSaveMessage('自动保存失败: ' + data.message, false);
                                    }
                                })
                                .catch(error => {
                                    console.error('自动保存失败:', error);
                                    showAutoSaveMessage('自动保存失败，请检查网络连接', false);
                                });
                        }
                    }, 3000); // 3秒后自动保存
                });
